...
#
# Regime logic:
#   BULL/SIDEWAYS → BUY signals for all FnO stocks
#   BEAR          → SELL signals only, FnO stocks, A/A+ grade
# ============================================================
...
def get_fyers_headers():
    return {"Authorization": f"{get_app_id()}:{get_access_token()}", "Content-Type": "application/json"}
...
def fetch_historical(symbol, days=400):
    try:
        ...
        params   = {
            "symbol": f"NSE:{symbol}-EQ", "resolution": "D",
            "date_format": "1", "cont_flag": "1",
            "range_from": start_dt.strftime("%Y-%m-%d"),
            "range_to":   end_dt.strftime("%Y-%m-%d"),
        }
        res  = requests.get("https://api-t2.fyers.in/research/history", params=params, headers=get_fyers_headers(), timeout=15)
        data = res.json()
...
def fetch_today_ohlc(symbols):
    ...
    try:
        ...
        for i in range(0, len(symbols), 50):
            ...
            res     = requests.get("https://api-t2.fyers.in/quotes/", params={"symbols": sym_str}, headers=get_fyers_headers(), timeout=15)
            data    = res.json()
            if data.get("s") != "ok":
                continue
            for q in data.get("d", []):
                ...
                v   = q.get("v", {})
                prices[sym] = {"open": v.get("open_price",0), "high": v.get("high_price",0), "low": v.get("low_price",0), "close": v.get("lp",0), "date": today_str}
    except Exception as e:
...
def hmm_regime(closes):
    try:
        ...
        for seed in range(3):
            try:
                m = GaussianHMM(n_components=3,covariance_type="diag",n_iter=80,tol=1e-2,random_state=seed,init_params="stmc")
...
def make_signal(sym, sig_type, grade, votes, fs, sp, rr, tp, closes, regime, vr, ev):
    ...
    if sig_type == "buy":
        ...
        wr  = 49
    return {
        "symbol": sym, "signal_type": sig_type, "grade": grade, "votes": votes,
        ...
        "market_regime": regime, "vol_regime": vr, "is_fno": True,
        "historical_win_rate": wr,
        "model_votes": {
            "factor":      {"active": True, "vote": 1},
            "regime":      {"active": True, "vote": 1},
        ...
        "backtest": {"total_trades": 100, "win_rate": wr/100}
    }
...
def run_full_scan():
    ...
    if bear_mode:
        SCAN_LOG.append("Bear regime — BUY blocked | FnO SELL signals only (A/A+)")
    else:
        SCAN_LOG.append(f"{regime.upper()} regime — BUY signals active")
    ...
    for sym in FNO_STOCKS:
        try:
            ...
            if bear_mode:
                sells.append(make_signal(sym,"sell",grade,votes,fs,sp,rr,tp,closes,regime,vr,ev))
            else:
                buys.append(make_signal(sym,"buy",grade,votes,fs,sp,rr,tp,closes,regime,vr,ev))
    ...
    with lock:
        SCAN_RESULTS = {
            ...
            "scan_metadata": {
                ...
                "buy_signals_count": len(buys), "sell_signals_count": len(sells),
                "market_regime": regime, "scan_duration_seconds": dur, "failed_stocks": failed
            }
        }
        save_scan()
        ...
        for sig in buys + sells:
            if any(t["symbol"]==sig["symbol"] and t["scan_date"]==today and t["signal_type"]==sig["signal_type"] for t in TRADE_HISTORY):
                continue
            TRADE_HISTORY.append({
                "id": str(uuid.uuid4()), "symbol": sig["symbol"], "signal_type": sig["signal_type"],
                "grade": sig["grade"], "score": sig["score"], "confidence": sig["confidence"],
                "votes": sig["votes"], "entry": sig["entry"], "stop_loss": sig["stop_loss"],
                "target": sig["target"], "risk_reward": sig["risk_reward"], "stop_pct": sig["stop_pct"],
                "market_regime": regime, "vol_regime": sig["vol_regime"], "is_fno": True,
                "model_votes": sig["model_votes"], "historical_win_rate": sig["historical_win_rate"],
                "scan_date": today, "scan_time": ts, "outcome_status": "pending",
                "entry_met": False, "entry_met_date": None, "actual_exit": None,
                "actual_pnl": None, "outcome_date": None,
                "expiry_date": (date.today()+timedelta(days=7)).isoformat(), "last_rescanned": None,
            })
            added += 1
        if added:
            save_history()

    SCAN_LOG.append(f"Done: {len(buys)} buys | {len(sells)} sells | {scanned} scanned | {failed} failed | {dur}s")
    log.info(f"Scan done: {len(buys)} buys, {len(sells)} sells in {dur}s")
...
def run_daily_rescan():
    ...
    if not get_access_token():
        log.error("No Fyers token — rescan aborted")
        return
    pending = [t for t in TRADE_HISTORY if t.get("outcome_status") not in ("target_hit","stop_hit","expired")]
    if not pending:
        log.info("No pending trades")
        return
    ...
    with lock:
        for trade in TRADE_HISTORY:
            ...
            entry = float(trade["entry"]); sl = float(trade["stop_loss"]); tgt = float(trade["target"])
            expiry= trade.get("expiry_date",""); st = trade.get("signal_type","buy")
            trade["last_rescanned"] = today
            if not trade.get("entry_met") and expiry and today > expiry:
                trade["outcome_status"] = "expired"; trade["outcome_date"] = today; updated += 1; continue
            if not trade.get("entry_met"):
                met = (low<=entry) if st=="buy" else (high>=entry)
                if met:
                    trade["entry_met"]=True; trade["entry_met_date"]=today; trade["outcome_status"]="entry_met"; updated+=1
            if trade.get("entry_met"):
                if st == "buy":
                    if high >= tgt:
                        trade.update({"outcome_status":"target_hit","actual_exit":tgt,"actual_pnl":round(((tgt-entry)/entry)*100,2),"outcome_date":today}); updated+=1
                    elif low <= sl:
                        trade.update({"outcome_status":"stop_hit","actual_exit":sl,"actual_pnl":round(((sl-entry)/entry)*100,2),"outcome_date":today}); updated+=1
                else:
                    if low <= tgt:
                        trade.update({"outcome_status":"target_hit","actual_exit":tgt,"actual_pnl":round(((entry-tgt)/entry)*100,2),"outcome_date":today}); updated+=1
                    elif high >= sl:
                        trade.update({"outcome_status":"stop_hit","actual_exit":sl,"actual_pnl":round(((entry-sl)/entry)*100,2),"outcome_date":today}); updated+=1
        if updated:
            save_history()
...
def api_token_exchange():
    try:
        ...
        app_id = get_app_id(); secret = get_secret_key()
        ...
        res    = requests.post("https://api-t2.fyers.in/api/v3/validate-authcode", json={"grant_type":"authorization_code","appIdHash":app_hash,"code":auth_code}, timeout=15)
        result = res.json()
        if result.get("s") != "ok":
            return jsonify({"error": result.get("message","Token exchange failed")}), 400
        save_access_token(result.get("access_token",""))
        return jsonify({"status":"success","message":"Token saved. Scanner ready for today."})
    except Exception as e:
...
def api_token_status():
    ...
    try:
        if os.path.exists(TOKEN_FILE):
            tok_date = json.load(open(TOKEN_FILE)).get("date")
    except:
        pass
    return jsonify({"token_set":bool(token),"token_date":tok_date,"app_id_set":bool(get_app_id()),"secret_set":bool(get_secret_key())})
...
def api_status():
    ...
    return jsonify({
        "status": "running", "last_scan": m.get("timestamp","Never"),
        "buy_signals": m.get("buy_signals_count",0), "sell_signals": m.get("sell_signals_count",0),
        "market_regime": m.get("market_regime","unknown"), "stocks_scanned": m.get("stocks_scanned",0),
        "history_count": len(TRADE_HISTORY), "next_scan": str(next_job.next_run_time) if next_job else "unknown",
        "scan_log": SCAN_LOG[-5:], "token_ready": bool(get_access_token()),
    })
...
def api_history_update():
    try:
        data = request.get_json(force=True); tid = data.get("id")
...
def api_rescan_manual():
    if not get_access_token():
        return jsonify({"error":"No Fyers token. Visit /token to generate today's token first."}), 400
    threading.Thread(target=run_daily_rescan, daemon=True).start()
    return jsonify({"status":"started","message":"Rescan running — refresh in 30 seconds"})

@app.route("/api/scan/manual", methods=["POST"])
def api_scan_manual():
    if not get_access_token():
        return jsonify({"error":"No Fyers token. Visit /token first."}), 400
    threading.Thread(target=run_full_scan, daemon=True).start()
    return jsonify({"status":"started","message":"Scan running — check /api/status"})
...
def api_upload():
    ...
    try:
        ...
        with lock:
            SCAN_RESULTS = {"buy_signals":data.get("buy_signals",[]),"sell_signals":data.get("sell_signals",[]),"scan_metadata":data.get("scan_metadata",{})}
